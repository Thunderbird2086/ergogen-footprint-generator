...
def setup_logger():
    ...
    formatter = logging.Formatter(
        '%(asctime)s - %(name)20s:%(lineno)5d - %(levelname)-8s - %(message)s')
    # Add formatter to ch
...
def parse_kicad_mod(kicad_mod_file, outdir):
    ...
    def modify_pad(pad):
        ...
        pad_id = pad_id.strip('"')
        pad_name = f'P{pad_id}' if pad_id[0].isdigit() else pad_id

        _LOGGER.debug("%s - %s", pad, pad_name)
        params.add(pad_name)
        pad.append("${"+f"p.{pad_name}"+"}")

        return pad

    def rebuild_mod_data(parsed_data):
        result = []
        ...
        if 'at' == result[0]:
            ...
            result = modify_pad(result)

        _LOGGER.debug(result)

        return f"({result[0]} {' '.join(map(str, result[1:]))})"

    def flatten_to_second_level(parsed_data):
    ...
    def filters_out(a_target, flattened, an_option=None):
        unprocessed = []
        filtered = []
        for item in flattened:
            ...
            if an_option and an_option not in item:
                unprocessed.append(item)
                continue
    ...
    def get_layers(flattened):
        targer_layers = ['F.Cu', 'B.Cu',
            'pad',
            'F.SilkS', 'B.SilkS',
            'F.Fab', 'B.Fab',
            'F.Mask', 'B.Mask',
        ...
        options = {'F.Cu': 'pad', 'B.Cu': 'pad'}
            
        layers = {}
        for a_layer in targer_layers:
            filtered, flattened = filters_out(
                a_layer, flattened, options.get(a_layer, None))
    ...
    def print_stats(layers, flattened):
        ...
        _LOGGER.info("Total       :%6d", total)
        _LOGGER.info("Unprocessed :%6d", len(layers['Unprocessed']))

    def get_code_blocks(layers):
        target_layers = {
            'Unprocessed': ('standard_opening', '(',
    ...
    def dump_ergogen_footprint(codeblock, kicad_mod_file, outdir):
...
if __name__ == "__main__":
    ...
    if not os.path.exists(args.outdir):
        ...
        _LOGGER.info("'%s' has been created as it did not exist.", outdir)
